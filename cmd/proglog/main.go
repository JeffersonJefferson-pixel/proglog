...
func (c *cli) setupConfig(cmd *cobra.Command, args []string) error {
	...
	c.cfg.StartJoinAddrs = viper.GetStringSlice("start-joni-addrs")
...
func (c *cli) run(cmd *cobra.Command, args []string) error {
	...
	// handle signal from operating system
	sigc := make(chan os.Signal, 1)
	signal.Notify(sigc, syscall.SIGINT, syscall.SIGTERM)
	<-sigc
	// shutdown agent when os terminates the program
...
func setupFlags(cmd *cobra.Command) error {
	...
	cmd.Flags().String("bind-addr", "127.0.1:8401", "Address to bind Serf on.")
	...
	cmd.Flags().String("server-tls-ca-file", "", "Patht o server certificate authority.")
