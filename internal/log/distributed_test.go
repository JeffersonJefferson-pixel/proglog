...
func TestMultipleNodes(t *testing.T) {
	...
	for i := 0; i < nodeCount; i++ {
		...
		require.NoError(t, err)
		if i != 0 {
			// add other servers to raft cluster.
			err = logs[0].Join(
	...
	for _, record := range records {
		...
		// check that raft replicated records to followers.
	...
	// remove one server from raft cluster.
	err = logs[0].Leave("1")
	require.NoError(t, err)
	...
	// check that leader stops replicating to server that has left the cluster.
	time.Sleep(50 * time.Millisecond)
